...
class PPOLearner(ActorCriticLearner):
    ...
    def get_batch(self):
        ...
        for i, (sample, action, value, policy, reward) in enumerate(memory):
            ...
            y_policy[i, :] = policy
            # Clipping
            old_policy = y_policy[i, action]
            new_policy = utils.sigmoid(advantage)
            ratio = np.exp(new_policy - old_policy)
            if (ratio >= 1.0 - clip_range) and (ratio <= 1.0 + clip_range):
                clipped_ratio = ratio
            elif ratio < 1.0 - clip_range:
                clipped_ratio = 1.0 - clip_range
            elif ratio > 1.0 + clip_range:
                clipped_ratio = 1.0 + clip_range
            # Clipping으로 믿을 수 있는 부분만 update
            y_policy[i, action] = clipped_ratio * utils.sigmoid(advantage)
            value_max_next = value.max()
        return x, y_value, y_policy